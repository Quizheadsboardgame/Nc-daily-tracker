...
export const SalesList: React.FC<SalesListProps> = ({
  ...
  const filteredSales = useMemo(() => {
    ...
    if (selectedSalesman !== 'all') {
      result = result.filter((s) => s.salesmanName === selectedSalesman);
    }
    ...
    return result;
  }, [sales, selectedPaymentFilter, selectedSalesman, searchQuery, sortBy]);
    ...
    <div className="bg-white rounded-2xl border border-zinc-200/90 shadow-xs overflow-hidden" id="card-sales-ledger">
      ...
      {filteredSales.length === 0 ? (
        <div className="p-12 text-center">
          ...
          {(selectedPaymentFilter !== 'all' || selectedSalesman !== 'all' || searchQuery) && (
            <button
              type="button"
              onClick={() => {
                onSelectPaymentFilter('all');
                onSelectSalesman('all');
                setSearchQuery('');
              }}
            ...
            return (
              ...
              >
                ...
                <div className="md:col-span-3 flex items-center gap-2">
                  <div className="w-7 h-7 rounded-full bg-zinc-200 flex items-center justify-center text-zinc-700 text-xs font-bold shrink-0">
                    {sale.salesmanName.charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <span className="text-xs font-bold text-zinc-900 block">{sale.salesmanName}</span>
                    <span className="text-[10px] text-zinc-400">Sales Representative</span>
                  </div>
                </div>
                ...
                <div className="md:col-span-4 space-y-1">
                  ...
                  {sale.paymentMethod === 'trade' && sale.tradeDetails && (
                    <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-amber-50 border border-amber-200 text-amber-900 text-[11px]">
                      <ArrowLeftRight className="w-3 h-3 text-amber-700 shrink-0" />
                      <span>Trade-in: {sale.tradeDetails}</span>
                    </div>
                  )}
